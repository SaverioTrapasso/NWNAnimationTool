...
func _ready() -> void:
	...
	viewport_toolbar.get_node("View3DButton").pressed.connect(func(): view_mode_requested.emit("3d"))
	viewport_toolbar.get_node("ViewFrontButton").pressed.connect(func(): view_mode_requested.emit("front"))
...
func _source_xform_spins(container: Node, kind: String) -> Array:
	if kind == "glb":
		return [container.get_node("XformRow/RotX"), container.get_node("XformRow/RotY"),
			container.get_node("XformRow/RotZ"), container.get_node("XformRow2/OffX"),
...
## "glb"): full XYZ rotation plus world offset, applied to the reference
## skeleton preview and baked into Apply/Bake.
func get_source_xform(kind: String) -> Transform3D:
	var container := _source_xform_container(kind)
	if container == null:
...
## Rotation as Euler radians, for consumers that set node.rotation directly
## (the glb source scene root).
func get_source_rotation(kind: String) -> Vector3:
	var container := _source_xform_container(kind)
	if container == null:
		return Vector3.ZERO
	var spins := _source_xform_spins(container, kind)
	return Vector3(deg_to_rad(spins[0].value), deg_to_rad(spins[1].value), deg_to_rad(spins[2].value))
