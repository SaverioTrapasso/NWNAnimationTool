...
static func bake(parent: Node, anim_glb_path: String, bone_map: Dictionary, nwn_rest_transforms_by_name: Dictionary, source_fps: float, root_scale: float, lock_data: Dictionary, flip_180: bool = false, source_rotation: Vector3 = Vector3.ZERO, source_offset: Vector3 = Vector3.ZERO) -> Dictionary:
	...
	# was pressed: flip + the user's manual SOURCE TRANSFORM (rotation, offset).
	anim_scene.rotation = source_rotation + Vector3(0.0, PI if flip_180 else 0.0, 0.0)
